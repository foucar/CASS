 ...
 */
void read(const std::string & filename, std::vector<float>& matrix,
          std::pair<int,int>& shape)
{
  std::ifstream cbf_file(filename.c_str(), std::ios::in | std::ios::binary);

  std::stringstream eoh;
  eoh << 0x0C << 0x1A << 0x04 << 0xD5;
  while (!cbf_file.eof())
  {
    std::string line;
    std::getline(cbf_file,line);
    if (line == eoh.str())
      break;
  ...
  {
    cbf_file.read(&onebyte,1);
    if (onebyte == -128)
    {
      cbf_file.read(&twobytes,2);
      if (twobytes == -32768)
      {
        cbf_file.read(&fourbytes,4);
        cVal += fourbytes;
      }
      else
        cVal += twobytes;
