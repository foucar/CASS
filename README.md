...
to analyze and visualize data taken at Free-electron-lasers (FELs). To be easily
adaptable to the various different types of experiments, it is based
on a highly modular design. The analysis part of CASS allows the user to set
up an analysis chain that can be tailored for each experimental need.
...
still developed in svn and are therefore outdated. They are only available for
historical reasons.
...
An anonymous clone of the latest master branch of the repository is accessible
via GitHub
...
-------------
See the [INSTALL.md](@ref cassinstall) file for the prerequisites. Once you have
...
Now modify the cass_myconfig.pri file to include what you need and
...
install it into the location that you requested with the optional PREFIX
parameter by typing:
...
By enabling the corresponding options in the cass_myconfig.pri file one can
enable CASS to compile the
...
use doxygen there e.g. if you are in the CASS base directory do:

    cd doc
    make doc

This will create a subdirectory of the doc directory called doxygen/html.
Within this directory you will find the html version of the documentation. Open
index.html with your favorite browser to view the documentation.
...
data stream) and offline (get the data from the written files).
...
A complete list of all available command-line parameters for the compiled mode
...
In this location you will also find some convenient startup scripts, as well
as binaries of older versions of CASS.

To end the program you should use:

    crtl + \

Killing the program with `crtl + c` had the potential to consume communication
buffers for the interface to the shared memory communication with LCLS. This
should be under control but be advised that one might use up all the available
...
need to be put into a text file. Then provide the name of the text file to CASS
with the
...
Sometimes you don't want to have the rate displayed, e.g. when you run
...
---------------------------
CASS consists of a lot of processing modules that one can put together to
process the data as you want it. These modules are called "Processors".
For better understanding of this concept, here is a short overview of the
program flow:

for each event:

- receive an event containing all data from either the file (offline) or
  the live data stream (online)
- convert the content to CASSEvent
- multiple worker will retrieve the CASSEvents in parallel and
...
allow to output results of an analysis into tables. There are Processors that
allow manipulation of the table outputs.
...
Where the Delayline detectors are Time of Flight detectors with additional
spatial information. Everything that a Time of Flight detector can do, a
...
The SignalProducer contains a SignalExtractor object that knows
...
specific SignalExtractor for its available parameters.
...
detector hits. You can choose which one of these methods you want to use with
the @c AnalysisMethod parameter. Please refer to the description of the specific
DetectorAnalyzer to see what parameters are available.

Once you have set up the DLD to identify hits, these hits can be assigned to
certain particles.  There are several ways of identifying which hit
belongs to which particle. You can choose which one to use with the
@c ConditionType parameter of the particle to define which method to use. For
the other parameters available for a particle please refer to the documentation
of cass::ACQIRIS::Particle.

In order to calculate the momenta and energy of the particles each particle
contains a CASS cass::ACQIRIS::Spectrometer object. You need to set up this
object for each particle individually. The particles momentum will be
calculated based upon the parameters you provide.
To calculate the momentum, the raw values of the detectorhit are taken and
corrected using a cass::ACQIRIS::HitCorrector object. Please refer to the
documentation of cass::ACQIRIS::HitCorrector to see which parameters are
available.
...
* Alternatively you can send patches to the principle developer.
...
----------------------------------
There are two versions of CASS available, the online and offline version. Both
...
The input modules will do the following tasks after they have retrieved the data
from whichever source that they get the data from:
1. get fillable (empty) cass::CASSEvent element from cass::RingBuffer
2. convert retrieved data to cass::CASSEvent data and fill them with the
   data.
...
- **sss:**
  ...
  contain concatenated raw image data where every image is preceded by the event
  id. No further parameters are available for this reader.

- **frms6:**
  These files are generated by the HLL data acquisition program and contain the
  frame data of the pnCCD detector. No further parameters are available for
  this reader.
...
##### HDF5 File input
As the HDF5 files are not ordinary binary files where the data is just
concatenated and one can parse them by serially reading the contents, a
...
HDF5 file, since it is not currently possible to combine information from
...
their tools to retrieve the data and let it be written into the HDF5 file or one
can use a provided API to access all data. This input modules makes use of the
...
The corresponding class is cass::SharedMemoryInput. It derives from the
...
from this server and parse it.
The data packets need to have the following layout: The first 32 bits should
contain the size of the following data packet. The following data packet then
needs to be deserialized according to the rules of the program that serves the
data.  Please refer to cass::TCPInput for more details and user choosable
parameters.
...
be ordered correctly such that all dependencies will be processed before the
...
will be passed to the individual Processors in that order. After the event
has been passed to all Processors by the manager, the manager instructs all
...
important functions that need to be overwritten are:

cass::Processor::loadSettings()
and
...
Please document what your processor does so that other people can understand its
function.  When documenting please use doxygen style since your documentation
will then be available on the Web server. Documenting the parameters in
cass.ini can be done using the custom doxygen tag cassttng.
...
cass::ConversionBackend::instance() and add an `else-if-statement` that returns
...
Updating it was just a matter of finding out which files have
...
library in a metalanguage that allows exporting it to different computer
...
The same is true for the XtcMonitorClient/Server/Msg files. The tricky part is
the BLD. Here one has to update the file
...
Push the tags to the repository server before pushing the changes in master.
Otherwise the tag number will not appear in the automatically generated
documentation.
...
Nicola Coppola
--------------
- cass_database (deprecated)
- cass_dictionary (deprecated)
- pnCCD analysis (deprecated)
- Region of Interest (ROI) implementation (deprecated)
- CASS testing, debug and development

Uwe Hoppe
---------
- old jocassview - SOAP client/imageviewer (deprecated)

Stephan Kassemeyer
-----------------
- Jocassview, plotwidget, spectrogramwidget (deprecated)
- general bug hunting and fixing.

Nils Kimmel
----------
- cass_pnCCD (deprecated)

Jochen Küpper
------------
- CASS design, infrastructure development
- cass framework implementation (deprecated)
- postprocessor setup and synchronization (deprecated)
...
Licence {#licence}
=======
CASS is developed under the terms of the GNU General Public
