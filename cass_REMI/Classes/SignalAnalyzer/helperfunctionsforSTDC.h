...
void fwhm(const Channel &c, Peak &p)
{
	...
	for (int i=p.maxpos(); i<wLength;++i)
	{
		if (abs(Data[i]-vOff) < HalfMax)
		{
			fwhm_r = i-1;
...
template <typename T>
void CoM(const Event &e, const Channel &c, Peak &p)
{
...
template <typename T>
void startstop(const Event &e, const Channel &c, Peak &p)
{
	//--this function will find the start and the stop of the peak--//
	const T * Data			= static_cast<const T*>(c.waveform());
	const long vOff			= static_cast<long>(c.vertOff()/c.vertGain());
	const long threshold	= c.threshold();
